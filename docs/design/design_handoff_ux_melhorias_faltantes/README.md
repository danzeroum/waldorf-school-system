...
## Parte 2 — Telas administrativas faltantes
Auditoria de **todos os módulos** do repositório. Estas telas administrativas não estavam na Parte 1. Mesmo formato: tela hifi + ficha técnica em `Telas Administrativas Faltantes.dc.html`.

### 4. Pessoas · Alunos (lista)
- **Arquivo:** `modules/pessoa/aluno/aluno-list/aluno-list.component.{html,ts}`
- **EXISTE:** `filtroSituacao()`, `mudarSituacao()`, `verDetalhe()`, paginação, skeleton, empty-state, pipe `situacaoAluno:'css'`.
- **NOVO:** trocar `<select>` de situação por **chips com contagem** (`contagemPorSituacao()` computed); coluna/subtítulo **Responsável** (`aluno.responsavelPrincipal` no DTO); ações diretas Ver/Editar + `more_vert` só para Transferir/Desligar.

### 5. Pessoas · Matrícula (wizard)
- **Arquivo:** `modules/pessoa/aluno/aluno-wizard/aluno-wizard.component.{html,ts}`
- **EXISTE (manter):** passos clicáveis (`irParaPasso`), `progresso`, `passoComErro(i)`, `formPasso1..5`, `form-input-error`/`form-error`, integração `busca-cep.service`. Etapas: Dados pessoais · Saúde · Responsáveis · Turma · Revisão.
- **NOVO:** trocar `max-w-2xl mx-auto` por grid `lg:grid-cols-[1.7fr_1fr]` com **painel de resumo fixo** à direita (lê `formPassoN.value` + progresso).

### 6. Pessoas · Aluno (ficha/detalhe)
- **Arquivo:** `modules/pessoa/aluno/aluno-detalhe/aluno-detalhe.component.{html,ts}`
- **EXISTE:** header de perfil, `calcularIdade()`, tabs (`tabAtiva` signal + `tabs[]`), aba Perfil completa.
- **NOVO:** implementar abas hoje "em construção": **Pedagógico** (`observacao.service` por aluno), **Financeiro** (`parcela.service` por aluno), **Frequência** (placeholder até endpoint).

### 7. Usuários & permissões — ⚠ MAIOR INCONSISTÊNCIA
- **Arquivo:** `modules/usuario/usuario-list/*` + `usuario-form/*`
- **Correções de consistência (prioridade):** remover `font-family:'Playfair Display'` → `font-serif` (Lora); remover wrapper `min-h-screen bg-waldorf-cream/30 p-6` (main-layout já dá padding); `mat-spinner` → `.skeleton`; `bg-green-100/red-100` → `.badge-success`/`.badge-danger`.
- **EXISTE:** `filtro`, `aplicarFiltro()`, `getPerfilColor/Label()`, `editar/resetarSenha/toggleAtivo/deletar()`, `isAdmin()`.
- **NOVO:** chips por perfil com `contagemPorPerfil()`; migrar `usuario-form` para slide-panel.

### 8. Financeiro · Contratos (lista)
- **Arquivo:** `modules/financeiro/contrato/contrato-list/contrato-list.component.{html,ts}`
- **EXISTE:** `filtroStatus()`, `verContrato()`, pipe `statusContrato:'css'`, skeleton.
- **NOVO:** select → **chips com contagem** (`contagemPorStatus()`); coluna **progresso de parcelas** (`parcelasPagas/totalParcelas` + barra); `border-l-red-500` em inadimplentes.

### 9. LGPD · Consentimentos
- **Arquivo:** `modules/lgpd/consentimento/consentimento-list/*` (+ `solicitacao/*`, `relatorio/*`)
- **EXISTE:** `statusOpcoes`, `filtrar()`, `filtroStatus()`, pipes `statusConsentimento`/`statusSolicitacao`.
- **NOVO:** **cards-resumo** no topo (`resumoConsentimentos()` → total/aceitos/pendentes/revogados); trocar `<table>` cru pelas classes `.table-header`/`.table-cell` e o badge ternário pelos pipes.

### Telas "PADRÃO ↓" (sem redesenho próprio)
Aplicar padrões já especificados, **sem inventar layout novo**:
- **Listas** (Professores, Turmas, Épocas) → padrão Parcelas/Alunos (busca + chips + `.table-*` + paginação).
- **Feeds/cards** (Mural, Comunicados, Notificações) → padrão Observações (`.card` com faixa de cor + filtros coloridos).
- **Dashboards de perfil** (Professor, Pais) → padrão Dashboard Secretaria (faixa de atenção + KPIs semânticos).
- **Formulários** (Comunicado, Época, Usuário…) → slide-panel do `turma-list`.
- **Login** → já OK; garantir `font-serif` no título e `.form-error` nos erros.
- **Consistência geral** → varredura por `Playfair Display`, `mat-spinner`, `p-6` avulso e cores cruas.

---

## Files (neste pacote)
- `Telas para Desenvolvimento.dc.html` — Parte 1: Dashboard, Parcelas, Observações + shell. **Referência principal.**
- `Telas Administrativas Faltantes.dc.html` — Parte 2: Alunos, Matrícula, Ficha, Usuários, Contratos, Financeiro, LGPD + inventário completo de telas.
- `Auditoria UX — Quintal Aroeira.dc.html` — comparativo antes → depois e justificativa.

> Os HTML abrem direto no navegador. Use-os como referência visual; implemente com as classes do design system do projeto.
